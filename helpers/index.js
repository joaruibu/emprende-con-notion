...
export const getAllItems = (dataType) => {
  if (!DATA_TYPE[dataType])
    return console.error(`No existe el tipo de dato ${dataType}`);
  ...
  if (dataType === DATA_TYPE.courses) return courses;
};
...
export const getItemById = (dataType, id) => {
  ...
  if (dataType === DATA_TYPE.templates)
    return templates.find((ele) => ele.id === id);
...
export const getFilterTemplates = (filterValue) => {
  const filterTemplates = templates.filter((template) =>
    template.title.toLowerCase().includes(filterValue.toLowerCase())
  );
