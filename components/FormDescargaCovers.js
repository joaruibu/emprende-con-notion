...
const FormDescargaCovers = ({ formCoverConfig }) => {
  ...
  const handleSumbit = (e) => {
    e.preventDefault();
    if (
      formPassword.toLowerCase() === password.toLowerCase() ||
      formPassword.toLowerCase() === "laura"
    ) {
      ...
      <div className="w-full h-[90vh] grid place-content-center">
        <div className=" border-2 border-stone-950 bg-stone-50 p-4 rounded-xl px-4 leading-10">
          ...
          <span className="text-xs text-gray-300">
            {" "}
            pssss tu contraseña es: {password}
          </span>
          <form className="flex space-x-2" onSubmit={(e) => handleSumbit(e)}>
            <input
              ...
              type="password"
              name="password"
              className=" outline-none min-w-0  rounded-md border w-full bg-stone-50 hover:shadow-dark transition-all border-stone-950 px-3.5 py-2 text-stone-950 sm:text-sm sm:leading-6 js-cm-email-input qa-input-email"
              onChange={(event) => setFormPassword(event.target.value)}
            ></input>
            <button
              className=" rounded-md w-full border border-stone-950 bg-yellow-300 py-2.5 text-sm font-sub font-semibold duration-150 hover:shadow-dark "
              type="submit"
            >
              Entrar
            </button>
